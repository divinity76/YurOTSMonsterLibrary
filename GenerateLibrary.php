...
$OTDataDir = __DIR__ . DIRECTORY_SEPARATOR . "data";
...
foreach ($db->query($query, PDO::FETCH_ASSOC) as $monster) {
    var_dump($monster);
    echo "\nlibrary is generated! folder for docroot: GeneratedHTML\n";
    die();
}
    ...
    (function ($db) use ($OTDataDir) {
        ...
        foreach ($monsterXMLs as $monsterXML) {
            ...
            if ($monsterDOMD->getElementsByTagName("health")->item(0)) {
                $ins_health_max = $monsterDOMD->getElementsByTagName("health")
                    ->item(0)
                    ->getAttribute('max');
            } else {
                $ins_health_max = '0';
            ...
            try {
                $ins_combat_targetdistance = $monsterDOMD->getElementsByTagName("combat")
                    ->item(0)
                    ->getAttribute('targetdistance');
            } catch (\Throwable $ex) {
                $ins_combat_targetdistance = 0;
            }
            try {
                $ins_combat_runonhealth = $monsterDOMD->getElementsByTagName("combat")
                    ->item(0)
                    ->getAttribute('runonhealth');
            } catch (\Throwable $ex) {
                $ins_combat_runonhealth = 0;
            }
            ...
            try {
                ...
                if (false !== strpos($ex->getMessage(), 'UNIQUE constraint failed: monsters.name')) {
                    // multiple monsters with the same name in monsters.xml ?
                    // FIXME: we should create a dedicated monster page for each monster with the same name,
                    // but as a quickfix i will just keep the first monster and ignore the rest of the monsters with the same name =/
                    continue;
                }
                throw $ex;
            }
...
function getImageThumbnailOfThing(string $thing, bool $useBrokenWikiaCode = false): string
{
    // Optimally we should parse the tibia.spr instead of this shit.. :/
    if (! $useBrokenWikiaCode) {
        static $monster_cache = null;
        static $item_cache = null;
        if ($monster_cache === null) {
            $monster_cache = json_decode(file_get_contents('monster_urls.json'), true);
        }
        if ($item_cache === null) {
            $item_cache = json_decode(file_get_contents('item_urls.json'), true);
        }
        $to_search = [
            $monster_cache,
            $item_cache
        ];
        foreach ($to_search as $arr_to_search) {
            foreach ($arr_to_search as $key_name => $key_val) {
                $name = $key_val["item_name"] ?? $key_val['monster_name'] ?? null;
                assert($name !== null, "data corruption...");
                if (0 === strcasecmp($thing, $name)) {
                    // found thing!
                    if (isset($key_val['monster_image_url'])) {
                        return $key_val['monster_image_url'];
                    } elseif (isset($key_val['item_image_url'])) {
                        return $key_val['item_image_url'];
                    } else {
                        throw new \LogicException('neither image url nor monster url!?');
                    }
                }
            }
        }
        ...
        echo "\nWARNING: trying to use broken wikia url fetching code (needs rewrite...)\n";
        ...
        $internalGetImageThumbnailOfThing = function ($thing) {
            ...
            $name = $thing;
            if (false !== stripos($domd->getElementById("PageHeader")->textContent, 'Redirected from')) {
                $name = trim($domd->getElementById("PageHeader")
...
function recurse_copy($src, $dst, $dirmode = 0775)
{
    $dir = ex::opendir($src);
    if (! is_dir($dst)) {
